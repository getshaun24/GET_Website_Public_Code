...
<p class="reverify_text">Based on your previously inputted values our KYC identity vendor requires that you reverify your information. Please double-check all entered information. </p>
...
<input v-model="reverify_email" class="input_black" placeholder=' ' required/>
<label class="label_black long_label">Email</label>
</div>
...
function page_counter_prev(){
    gsap.to('.modal_popup', {duration: .85, scrollTo: {y: ".progress_bar", offsetY: 20} });
...
function part_to_show(count){
    ...
    if(count == 3){
        ...
        if (dwolla_beneficial_owner_status.value != "incomplete"){
            next_text.value = "Submit"
        } else{
            next_text.value = "Next"
        }
...
function page_counter_next(){
    gsap.to('.modal_popup', {duration: .85, scrollTo: {y: ".progress_bar", offsetY: 20} });
    if(page_count.value == 4){
        step_3_reverify_send()
    }
    if(page_count.value < 4){
        if(page_count.value == 2 && ( investing_as.value == "individual" || investing_as.value == "soleProprietorship")) {
        step_3_reverify_send()
    } else if(page_count.value == 3 && ( dwolla_beneficial_owner_status.value != "incomplete")) {
        step_3_reverify_send()
    }
    else{
...
const data_to_send = {
    ...
    reverify_email: reverify_email.value,
...
console.error('There was an error no send');
    ...
    // open modal on load
    sleep(1500).then(() => {      // Do something after the sleep!
