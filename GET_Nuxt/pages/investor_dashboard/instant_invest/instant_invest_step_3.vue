<template>
    ...
    <GSAPScrollSmoother>
        ...
        <ManagerFundingAccountButtons v-model:funding_selected_value="funding_selected_value"/> 
...
const funding_selected_value = useCookie('funding_selected_value', cookie_options)
...
fetch(`${config.flask_url}/api/investor_dashboard/instant_invest/instant_invest_step_3/`, {
    method: 'POST',
    headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
    },
    body: JSON.stringify({investor_ID:investor_ID.value, fund_ID: fund_ID.value, institution_ID:institution_ID.value, investment_amount:investment_amount.value, funding_selected_value:funding_selected_value.value, account_selected_value_step_2:account_selected_value_step_2.value, email:email.value, phone:phone.value})
})
...
.then((data) => {
    ...
    if(data.status == 'Insufficient_Funds'){
        ...
        transition_and_route('instant_congradulations')
    }
    else{
