    ...
    <div class="dark_mode" style="overflow-x:hidden">
        ...
        <GSAPScrollSmoother>
            ...
            <div class="navigation_header">
                <div class="nav_menu_title">Transactions</div>
            ...
            <div class="dashboard_inner_section">
                <div class="main_content_container">
                  ...
                  :table_buttons="table_buttons"  :record_count="record_count" :table_row_style="table_row_style" />
                    ...
                    <p class="dwolla_bal" v-if="dwolla_balance != 0">Dwolla Balance: ${{ dwolla_balance }}</p>
                </div>
            </div>
...
onBeforeMount(() => { if (authCookieRefresh()) {get_transactions()} })

const cookie_options = {default:()=> '', watch:true, maxAge:1800}
const investor_ID = useCookie('investor_ID', cookie_options)

const active_page = {is_active: "transactions"}



const dwolla_balance = ref()



// ---------------------  Table Component Data --------------------- //
const table_name = ref('All Transactions')
const table_row_style = ref('transactions_table')
const table_headers = ref(['Date', 'Fund', 'Status', 'Amount', 'Transfer ID'])
const table_buttons = ref(['Failed', 'Pending', 'Completed'])
...
fetch(`${config.flask_url}/api/investor_dashboard/transactions/`, {
    method: 'POST',
    mode: 'cors',
    headers: {
...
.then((data) => {
    ...
    dwolla_balance.value = data.dwolla_balance
})
.catch(error => {
    // alert("Error")
